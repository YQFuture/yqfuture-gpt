...
func (m *customBsOrganizationModel) SessionInsert(ctx context.Context, data *BsOrganization, session sqlx.Session) (sql.Result, error) {
	...
	ret, err := session.ExecCtx(ctx, query, data.Id, data.OwnerId, data.OrgName, data.BundleType, data.MaxSeat, data.MonthPowerLimit, data.MonthUsedPower, data.MongoPermId, data.CreateBy, data.UpdateBy)
	return ret, err
}
